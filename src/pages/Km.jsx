...
function Km() {
  ...
  const handleSubmit=async(e)=>{
    ...
    try {
      ...
      const response=await fetch('https://car-price-prediction-api.onrender.com/predict-kms',{
        method:'POST',
        mode:'cors',
        cache:'no-cache',
        headers:{
      ...
      console.log(data.kms_driven)
      setPredictedPrice(data.kms_driven[0].toFixed(0)) 
      setGotResult(true)
    } catch (error) {
      console.log(error)
      setError(error)
...
{gotResult &&
  ...
  <div className='h-screen w-screen absolute top-0 left-0 flex justify-center items-center'>
    ...
    <h1 className='text-5xl font-bold'>Your you drove car :</h1>
    <h1 className='text-5xl font-bold m-6 text-yellow-600'>{predictedPrice} kms</h1>
