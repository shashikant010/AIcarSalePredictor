...
function year() {
  ...
  const handleSubmit=async(e)=>{
    ...
    try {
      ...
      const response=await fetch('https://car-price-prediction-api.onrender.com/predict-year',{
        method:'POST',
        mode:'cors',
        cache:'no-cache',
        headers:{
      ...
      setPredictedPrice(data.year[0].toFixed(0)) 
      setGotResult(true)
    } catch (error) {
      console.log(error)
      setError(error)
  ...
  <main className="flex justify-center items-center w-full h-full">
    <form action="post" className="border-black rounded-2xl border-4  w-1/2 mx-auto m-4 p-4 flex flex-col  bg-white bg-opacity-70 form items-start shadow-white shadow-3xl">
        <h1 className="text-3xl font-bold text-center text-blue-800 m-2 font-serif mb-4">AI CAR Year predictor</h1>
        ...
        <label htmlFor="sp" className="text-2xl font-bold">Enter For how much your car is sold</label>
        ...
        <label htmlFor="km-driven" className="text-2xl font-bold">Enter How much km your car is driven</label>
        <input type="text" name="km-driven" id="" placeholder="Km driven" className="h-8 w-96 text-2xl p-4 border-gray-500 rounded-xl border-2 m-2"
        value={kmDriven}
...
{gotResult &&
  ...
  <div className='h-screen w-screen absolute top-0 left-0 flex justify-center items-center'>
    ...
    <h1 className='text-5xl font-bold'>Your your car's Year is :</h1>
    <h1 className='text-5xl font-bold m-6 text-yellow-600'>{predictedPrice}</h1>
    <button className='text-white bg-lime-700 p-4 rounded-xl font-bold ' onClick={()=>navigate("/")}>Predict more</button>
    </div>
  </div>
  </>
  }
